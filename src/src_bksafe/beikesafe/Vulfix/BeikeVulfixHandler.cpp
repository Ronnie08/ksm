...
LRESULT CBeikeVulfixHandler::OnListLinkClicked( UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled )
{
	...
	if( theEngine && m_wndListCtrlVulFixing.IsWindowVisible() )
	{
		...
		if(pItem)
		{
			...
			CString strUrl;
			strUrl.Format(_T("http://www.update.microsoft.com/windowsupdate/v6/default.aspx?ln=zh-cn"), strName);
...
void CBeikeVulfixHandler::OnBkBtnErrorRepair()
{
	...
	if(pItemData)
	{
		::ShellExecute(NULL,_T("open"), _T("http://www.update.microsoft.com/windowsupdate/v6/default.aspx?ln=zh-cn"), NULL, NULL, SW_SHOW);
	}
}
...
void CBeikeVulfixHandler::OnBkBtnExport()
{
	...
	CFileDialog dlg(FALSE,_T("html"),_T("系统漏洞扫描结果.html"), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT,szFilters);
	if(dlg.DoModal()!=IDOK)
		return;
